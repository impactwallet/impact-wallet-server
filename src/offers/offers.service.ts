...
export class OffersService extends OffersServiceBase {
    ...
    async createOffer(orgId: string, offer: OfferDto) {
        if (offer.memberProspect.role === Role.Investor) {
            if (
                ...
                offer.memberProspect.investorSettings.minimalInvestment
            ) {
            ...
            const soldEquity = existingInvestors.reduce((res, member) => {
                return (
                    res + get(member, 'investorSettings.equityAllocation', 0)
                );
            }, 0);
            if (
                soldEquity +
            ...
            case OfferStatusDto.accepted:
                ...
                if (offer.memberProspects[0].role === Role.Investor) {
                    ...
                    const paymentInfo = {
                        ...
                        amount: offer.memberProspects[0].investorSettings
                            .investmentAmount
                    };
    ...
    async updateSaleOfferStatus(
        ...
        const offer = await this.getSaleOfferById(offerId, [
            { path: 'org' },
