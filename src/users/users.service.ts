...
export class UsersService extends UsersServiceBase {
  ...
  async getUsersByNicknamePrivate(name: string): Promise<User[]> {
    ...
    const users = await this.userRepository
      ...
      .exec();
    if (!users)
      throw new NotFoundException(`User with nickname '${name}' not found`);
    return users;
  }
  ...
  async updateUser(updateUserDto: UpdateUserDto, account: AccountModel) {
    const user = await this.userRepository.findById(
      new mongoose.Types.ObjectId(account.user._id),
    );
    if (isNil(user)) {
  ...
  async withdrawCredits(account: AccountModel, body: CreditsWithdrawDto) {
    if (isNil(body.recipient)) {
  ...
  async sendAssets(
    ...
    await session.withTransaction(async () => {
      ...
      this.apiService.sendNotification(
        `User ${account.username} sent ${sendAssetsDto.amount}% of equity in ${
          org.name
        } to ${get(
          recipient,
          'nickname',
  ...
  async _buildAssetHistory(
    ...
    for (const txn of parsedTxns) {
      ...
      if (!isNil(inAppEntity.org)) {
        ...
        const user = (
          amount < 0 ? inAppEntity.sale.buyer : inAppEntity.sale.seller
